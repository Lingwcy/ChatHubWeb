...
import { AllUserResultData } from "@/api/user";
...
export function useUser(tableRef: Ref, treeRef: Ref) {
  ...
  async function onbatchDel() {
    ...
    const payload: DeletesProps = { ids };
    await useUserStoreHook().postDeletesUser(payload);
      ...
      beforeSure: (done, { options }) => {
        ...
        FormRef.validate(async valid => {
          if (valid) {
            // 表单规则校验通过
            if (title === "新增") {
              let result = await useUserStoreHook().addUserByAdmin(curData);
              console.log(result);
              chores();
            } else {
              // 实际开发先调用修改接口，再进行下面操作
